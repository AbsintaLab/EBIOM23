...
object <- readRDS("data/schafflick_blood_harmony_by_origident.rds")
DimPlot(object,reduction = "umap",label = T)
ggsave("out/image/UMAP_schafflickBlood_tot.pdf",width = 7,height = 5)
...
#   write_tsv("out/table/schafflick_blood_CoordUMAP_curated.tsv")
# 
# object@reductions$umap@cell.embeddings %>%
#   data.frame() %>%
#   rownames_to_column("barcodes") %>%
...
# saveRDS(object_diet,"out/object/diet_schafflick_blood.rds")
query <- LoadFileInput(path = "out/object/diet_schafflick_blood.rds")

# schafflick_bloodl: for this dataset Martina asked to score all the cells, but to keep the one from cluster 4 and 9 which are immun cells
...
query@meta.data %>%
  rownames_to_column("barcodes") %>%
  write_tsv("out/table/schafflick_blood_metaAzimuth.tsv")

query@reductions$proj.umap@cell.embeddings %>%
  data.frame() %>%
  rownames_to_column("barcodes") %>%
  write_tsv("out/table/schafflick_blood_CoordUMAP_Azimuth.tsv")
# 
...
UMAP_query <- read_tsv("out/table/schafflick_blood_CoordUMAP_curated.tsv")
UMAP_query %>% 
  ggplot(aes(x=UMAP_1,y=UMAP_2))+geom_point()

meta_query <- read_tsv("out/table/schafflick_blood_metaAzimuth.tsv") %>%
...
ggsave("out/image/schafflick_blood_predicted.celltype.l1_070.pdf",height = 4,width = 4)
...
meta_query %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 45)) + 
  geom_hline(yintercept = 0.70,col="red")
ggsave("out/image/schafflick_blood_predicted.celltype.l2_070.pdf",height = 4,width = 4)
...
ggsave("out/image/schafflick_blood_predicted.celltype.l1_ridges_070.pdf",height = 5,width = 4)
...
meta_query %>%
  ...
  theme_bw() + 
  geom_vline(xintercept = 0.70,col="red",linetype="dashed")
ggsave("out/image/schafflick_blood_predicted.celltype.l2_ridges_070.pdf",height = 5,width = 4)
...
pdf("out/image/schafflick_blood_predicted.celltype.l1_heatmapAll.pdf",height = 3,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l1_heatmapAll2.pdf",height = 3,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l2_heatmapAll.pdf",height = 6,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l2_heatmapAll2.pdf",height = 6,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l1_heatmapFilter_070.pdf",height = 3,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l1_heatmapFilter2_070.pdf",height = 3,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l2_heatmapFilter_070.pdf",height = 6,width = 5)
...
pdf("out/image/schafflick_blood_predicted.celltype.l2_heatmapFilter2_070.pdf",height = 6,width = 5)
...
ggsave("out/image/schafflick_blood_predicted.celltype.l1_UMAP_all.pdf",width = 7,height = 5)
...
ggsave("out/image/schafflick_blood_predicted.celltype.l2_UMAP_all.pdf",width = 8,height = 5)
...
ggsave("out/image/schafflick_blood_predicted.celltype.l1_UMAP_robust_070.pdf",width = 7,height = 5)
...
ggsave("out/image/schafflick_blood_predicted.celltype.l2_UMAP_robust_070.pdf",width = 8,height = 5)
...
UMAP_ref_query <- read_tsv("out/table/schafflick_blood_CoordUMAP_Azimuth.tsv")
meta_ref <- read_tsv("data/human_PBMC_v1.0.0_metaAzimuth.tsv")
meta_ref_query <- read_tsv("out/table/schafflick_blood_metaAzimuth.tsv") %>%
  mutate(robust_score_celltype.l1 = case_when(predicted.celltype.l1.score>0.70&mapping.score>0.70~predicted.celltype.l1,
                                              T~"uncertain"),
         robust_score_celltype.l2 = case_when(predicted.celltype.l2.score>0.70&mapping.score>0.70~predicted.celltype.l2,
                                              T~"uncertain"),
...
ggsave("out/image/schafflick_blood_refPBMC_immune_predicted.celltype.l1_UMAP_all.pdf",width = 7,height = 5)
...
ggsave("out/image/schafflick_blood_refPBMC_immune_predicted.celltype.l1_UMAP_robust_070.pdf",width = 7,height = 5)
