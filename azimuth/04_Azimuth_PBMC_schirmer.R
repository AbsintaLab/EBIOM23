...
# reference <- LoadReference(path = "data/human_PBMC_v1.0.0/")
...
object <- readRDS("data/schirmer.rds")
DimPlot(object,reduction = "curated",label = T)
ggsave("out/image/UMAP_shirmer_tot.pdf",width = 7,height = 5)
# martina is interested also in the immune subset alone, which is cluster 19,7,8
object_imm <- subset(object, seurat_clusters %in% c(7,8,19), invert = F)
# save the imm subset
saveRDS(object_imm,"out/object/shirmer_imm.rds")
DimPlot(object_imm,reduction = "curated",label = T)
ggsave("out/image/UMAP_shirmer_imm.pdf",width = 6,height = 5)
# 
# # save the coordiantes of the UMAP
# object@reductions$curated@cell.embeddings %>%
#   data.frame() %>%
#   rownames_to_column("barcodes") %>%
#   write_tsv("out/table/schirmer_CoordUMAP_curated.tsv")
# 
# object_imm@reductions$curated@cell.embeddings %>%
#   data.frame() %>%
#   rownames_to_column("barcodes") %>%
#   write_tsv("out/table/schirmer_imm_CoordUMAP_curated.tsv")
# 
# object@reductions$curated@cell.embeddings %>%
#   data.frame() %>%
#   rownames_to_column("barcodes") %>%
...
# saveRDS(object_diet,"out/object/diet_schirmer.rds")
# saveRDS(object_imm_diet,"out/object/diet_schirmer_imm.rds")
query <- LoadFileInput(path = "out/object/diet_schirmer_imm.rds")

# schirmerl: for this dataset Martina asked to score all the cells, but to keep the one from cluster 4 and 9 which are immun cells
...
query@meta.data %>%
  rownames_to_column("barcodes") %>%
  write_tsv("out/table/schirmer_imm_metaAzimuth.tsv")

query@reductions$proj.umap@cell.embeddings %>%
  data.frame() %>%
  rownames_to_column("barcodes") %>%
  write_tsv("out/table/schirmer_imm_CoordUMAP_Azimuth.tsv")
# 
...
UMAP_query <- read_tsv("out/table/schirmer_imm_CoordUMAP_curated.tsv")
UMAP_query %>% 
  ggplot(aes(x=curated_1,y=curated_2))+geom_point()

meta_query <- read_tsv("out/table/schirmer_imm_metaAzimuth.tsv") %>%
...
ggsave("out/image/schirmer_imm_predicted.celltype.l1_070.pdf",height = 4,width = 4)
...
meta_query %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 45)) + 
  geom_hline(yintercept = 0.70,col="red")
ggsave("out/image/schirmer_imm_predicted.celltype.l2_070.pdf",height = 4,width = 4)
...
ggsave("out/image/schirmer_imm_predicted.celltype.l1_ridges_070.pdf",height = 5,width = 4)
...
meta_query %>%
  ...
  theme_bw() + 
  geom_vline(xintercept = 0.70,col="red",linetype="dashed")
ggsave("out/image/schirmer_imm_predicted.celltype.l2_ridges_070.pdf",height = 5,width = 4)
...
pdf("out/image/schirmer_imm_predicted.celltype.l1_heatmapAll.pdf",height = 3,width = 3)
...
pdf("out/image/schirmer_imm_predicted.celltype.l1_heatmapAll2.pdf",height = 3,width = 3)
...
pdf("out/image/schirmer_imm_predicted.celltype.l2_heatmapAll.pdf",height = 4,width = 3)
...
pdf("out/image/schirmer_imm_predicted.celltype.l2_heatmapAll2.pdf",height = 4,width = 3)
...
pdf("out/image/schirmer_imm_predicted.celltype.l1_heatmapFilter_070.pdf",height = 3,width = 3)
Heatmap(prop_predicted.celltype.l1_filter,
        name = "prop",
        column_title = "celltype l1 high confidence",
        cluster_rows = T,cluster_columns = T,col = viridis::turbo(100))
dev.off()

pdf("out/image/schirmer_imm_predicted.celltype.l1_heatmapFilter2_070.pdf",height = 3,width = 3)
Heatmap(matrix = prop_predicted2.celltype.l1_filter,
        name = "prop",
        column_title = "celltype l1 high confidence",
...
pdf("out/image/schirmer_imm_predicted.celltype.l2_heatmapFilter_070.pdf",height = 4,width = 3)
...
pdf("out/image/schirmer_imm_predicted.celltype.l2_heatmapFilter2_070.pdf",height = 4,width = 4)
Heatmap(prop_predicted2.celltype.l2_filter,
        name = "prop",
        column_title = "celltype l2 high confidence",
        cluster_rows = T,cluster_columns = T,col = viridis::turbo(20))
dev.off()
...
ggsave("out/image/schirmer_imm_predicted.celltype.l1_UMAP_all.pdf",width = 7,height = 5)
...
ggsave("out/image/schirmer_imm_predicted.celltype.l2_UMAP_all.pdf",width = 8,height = 5)
...
ggsave("out/image/schirmer_imm_predicted.celltype.l1_UMAP_robust_070.pdf",width = 7,height = 5)
...
ggsave("out/image/schirmer_imm_predicted.celltype.l2_UMAP_robust_070.pdf",width = 8,height = 5)
...
UMAP_ref_query <- read_tsv("out/table/schirmer_imm_CoordUMAP_Azimuth.tsv")
meta_ref <- read_tsv("data/human_PBMC_v1.0.0_metaAzimuth.tsv")
meta_ref_query <- read_tsv("out/table/schirmer_imm_metaAzimuth.tsv") %>%
  mutate(robust_score_celltype.l1 = case_when(predicted.celltype.l1.score>0.70&mapping.score>0.70~predicted.celltype.l1,
                                              T~"uncertain"),
         robust_score_celltype.l2 = case_when(predicted.celltype.l2.score>0.70&mapping.score>0.70~predicted.celltype.l2,
                                              T~"uncertain"),
...
ggsave("out/image/schirmer_imm_refPBMC_immune_predicted.celltype.l1_UMAP_all.pdf",width = 7,height = 5)
...
ggsave("out/image/schirmer_imm_refPBMC_immune_predicted.celltype.l1_UMAP_robust_070.pdf",width = 7,height = 5)
