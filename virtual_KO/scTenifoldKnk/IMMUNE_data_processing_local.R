...
# martina is interested in subsetting only the cells belonging ot the c_chronic_active
...
#
dim(id_barcodes)
# at the moment martina is asking to check all the immune cells alltogether
# potentially we could consider trimming same cell types 
...
# # define the grouping of the cells
# id_ms <- meta %>%
#   filter(disease == "ms") %>%
#   pull(barcode)
# #
...
# # save the table for only ms
# test_microglia_raw_data %>%
#   dplyr::select("gene",id_ms) %>%
#   write_tsv("data/MG_martina_raw_data_ms.tsv")
# 
# test_microglia_raw_data %>%
#   dplyr::select("gene",id_ms) %>%
#   dplyr::filter(id_trimming) %>%
#   write_tsv("data/MG_martina_raw_data_ms_trimm.tsv")
# 
# # save the table for ctrl
# test_microglia_raw_data %>%
#   dplyr::select("gene",id_ctrl) %>%
#   write_tsv("data/MG_martina_raw_data_ctrl.tsv")
# 
# test_microglia_raw_data %>%
#   dplyr::select("gene",id_ctrl) %>%
#   dplyr::filter(id_trimming) %>%
#   write_tsv("data/MG_martina_raw_data_ctrl_trimm.tsv")