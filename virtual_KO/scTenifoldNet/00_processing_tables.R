...
# test <- read_tsv("data/ALL_martina_meta.tsv")
# table(test$pathology)
...
# martina also asked to try to remove the cluster 7 (CD4 cells)
id_Tcells <- meta %>%
...
# martina also asked to try to remove the cluster 1
id_mim1 <- meta %>%
...
# martina also asked to try to remove the cluster 8
id_mim8 <- meta %>%
...
# Martina asked also to check the expression of CD20 and simulate the removal of the only those non zero expression cells
df_IMM_CCA$gene %>%
...
# martina also asked to do the same of CD20 to BTK 
df_IMM_CCA$gene %>%
...
# Martina also asked to check the removal of CD69 from T cells
...
# save the noremalized version of the tables
df_IMM_CCA_norm %>%
